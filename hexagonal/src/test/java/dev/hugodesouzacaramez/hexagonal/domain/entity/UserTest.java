package dev.hugodesouzacaramez.hexagonal.domain.entity;
...
public class UserTest {
    ...
    public void givenTheUserEmailAlreadyExistsAnExceptionIsThrown() {
        var user = new User("test@hugodesouzacaramez.dev", "password");
    ...
    public void giveThePasswordIsNotCorrectTheAuthenticationFail() {
        var user = new User("test@hugodesouzacaramez.dev", "password");
        var optionalUser = Optional.of(new User("test@hugodesouzacaramez.dev", "wrongPassword"));
        var loginResult = user.login(optionalUser);
